...
def _ascii_safe(s):
    """Replace non-ASCII chars so DXF is safe."""
    return (str(s)
            .replace('²', '2')
            .replace('³', '3')
            .replace('°', 'deg')
...
def build_complete_project(params):
    ...
    # ----- 5. DXF setup — R2000 with standard linetypes preloaded -----
    ...
    # ----- 7. Outer walls -----
    wall_ext_t = 250
    msp.add_lwpolyline(_wall_rect((x0, y0), (x0 + L, y0), wall_ext_t),
                       dxfattribs={'layer': 'A-WALL-EXT', 'color': 7})
    msp.add_lwpolyline(_wall_rect((x0 + L, y0), (x0 + L, y0 + W), wall_ext_t),
                       dxfattribs={'layer': 'A-WALL-EXT', 'color': 7})
    msp.add_lwpolyline(_wall_rect((x0 + L, y0 + W), (x0, y0 + W), wall_ext_t),
                       dxfattribs={'layer': 'A-WALL-EXT', 'color': 7})
    msp.add_lwpolyline(_wall_rect((x0, y0 + W), (x0, y0), wall_ext_t),
                       dxfattribs={'layer': 'A-WALL-EXT', 'color': 7})
    ...
    for room, (rx0, ry0, rx1, ry1) in placements:
        if rx0 > x0 + wall_ext_t:
            wall_segments.append(((rx0, ry0), (rx0, ry1), int_t))
        if rx1 < x1 - wall_ext_t:
            wall_segments.append(((rx1, ry0), (rx1, ry1), int_t))
        if ry0 > y0 + wall_ext_t:
            wall_segments.append(((rx0, ry0), (rx1, ry0), int_t))
        if ry1 < y1 - wall_ext_t:
            wall_segments.append(((rx0, ry1), (rx1, ry1), int_t))

    for p1, p2, t in wall_segments:
    ...
    title_lines = [
        ...
        (f"PLOT: {plot_area:.1f} m2   FLOORS: {num_floors}   COVERAGE: {coverage*100:.0f}%", tb_y+tb_h-1400),
        ("DRAWING: GROUND FLOOR PLAN + STRUCTURE", tb_y+tb_h-1750),
        ("SCALE: 1:100 (mm)   UNITS: mm", tb_y+tb_h-2100),
        ("EGYPTIAN CODE: LAW 119/2008 - ECP 203", tb_y+tb_h-2450),
    ]
    for txt, yy in title_lines:
        t = msp.add_text(_ascii_safe(txt),
                         dxfattribs={'layer': 'ANNO-TEXT', 'height': 180, 'color': 7})
    ...
    total_beam_length = (nx-1)*ny*(L/(nx-1) if nx>1 else 0) + (ny-1)*nx*(W/(ny-1) if ny>1 else 0)
    ...
    boq_items = [
        {'Item': 'Concrete (m3)', 'Quantity': round(total_concrete, 2), 'Unit': 'm3'},
        {'Item': 'Rebar (ton)',    'Quantity': round(rebar_ton, 2), 'Unit': 'ton'},
        {'Item': 'Formwork (m2)', 'Quantity': round(formwork, 2), 'Unit': 'm2'},
        {'Item': 'Bricks (nos)',   'Quantity': round(brick_count), 'Unit': 'nos'},
        {'Item': 'Flooring (m2)', 'Quantity': round(flooring_area, 2), 'Unit': 'm2'},
        {'Item': 'Paint (m2)',    'Quantity': round(paint_area, 2), 'Unit': 'm2'},
    ...
    # ----- 18. Write DXF as BINARY (universal AutoCAD compatibility) -----
    dxf_buf = io.BytesIO()
