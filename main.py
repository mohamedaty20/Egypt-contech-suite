...
# DXF FUNCTIONS (FIXED)
# =====================================================================================
def read_dxf(data_bytes):
    """
    Robust DXF reader that handles encoding issues.
    First tries binary read, then falls back to text with 'latin1' encoding.
    """
    # Try with default binary read
    try:
        return ezdxf.read(io.BytesIO(data_bytes))
    except Exception as e:
        print(f"Binary DXF read failed: {e}")
        # Fallback: try to decode as text (latin1) and read from StringIO
        try:
    ...
    with ui.column().classes('w-full min-h-screen p-4 bg-[#031338]'):
        ...
        with ui.tab_panels(tabs, value=t_dash).classes('w-full bg-transparent mt-4'):
            ...
            with ui.tab_panel(t_dash):
                # ... (copy from your previous working code – unchanged)
                pass

            # TAB 2-6: ... (unchanged, keep your existing code for these tabs)
            # For brevity, I'll include a placeholder but you must use your original code.
            # Please ensure you copy your working tabs from the previous version.
            ...
            with ui.tab_panel(t_dxf):
                ...
                async def process_dxf():
                    ...
                    try:
                        doc = read_dxf(dxf_file_data['bytes'])
