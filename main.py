...
def scrape_jsearch(query, max_results=20):
    jobs = []
    if not RAPIDAPI_KEY:
        print("[JSearch] ⚠️ RAPIDAPI_KEY is not set. Skipping.")
        return jobs
    ...
    params = {
        ...
        "num_pages": "1",
        "country": "eg",
        "engine": "google_jobs",   # explicitly ask for Google Jobs
    }
    headers = {
        "X-RapidAPI-Key": RAPIDAPI_KEY,
        "X-RapidAPI-Host": JSEARCH_HOST,
    ...
    try:
        ...
        print(f"[JSearch] status={resp.status_code}, len={len(resp.text)}")
        if resp.status_code != 200:
            print(f"[JSearch] error response: {resp.text[:200]}")
            return jobs

        data = resp.json()
        if "data" not in data:
            print(f"[JSearch] unexpected JSON structure: {list(data.keys())}")
            return jobs
        ...
        for item in data["data"][:max_results]:
            ...
            country = item.get("job_country") or "Egypt"
            location = ", ".join(p for p in [city, country] if p) or "Egypt"
            description = (item.get("job_description") or "No description")[:400]
            url_job = item.get("job_apply_link") or item.get("job_google_link") or ""
            source = item.get("job_publisher") or "JSearch"
            if not url_job:
                continue
            jobs.append({
...
# ---------- FALLBACK: Direct Wuzzuf scraper (stable selector) ----------
...
def scrape_wuzzuf_direct(query, max_results=20):
    """Direct Wuzzuf parser using the stable `/jobs/p/` link pattern."""
    jobs = []
    url = f"https://wuzzuf.net/search/jobs/?q={quote_plus(query)}&a=hpb"
    ...
    for a in soup.select('a[href*="/jobs/p/"]'):
        ...
        # Walk up to the parent that has at least two links (title + company)
        card = a
        for _ in range(6):
            card = card.parent
            if card is None:
...
def scrape_jobs(query, location=""):
    ...
    try:
        jobs = scrape_jsearch(full_query)
        if jobs:
            all_jobs.extend(jobs)
    except Exception as e:
        print(f"[scraper] JSearch error: {e}")

    # 2. If JSearch returned nothing, fall back to direct Wuzzuf
    if not all_jobs:
        print("[scraper] JSearch returned empty – falling back to direct Wuzzuf.")
        try:
            all_jobs.extend(scrape_wuzzuf_direct(full_query))
        except Exception as e:
            print(f"[scraper] Wuzzuf fallback error: {e}")

    # 3. (Optional) Add Bayt/Forasna/Akhtaboot if you want – but Wuzzuf is usually enough.
    #    You can uncomment the other scrapers if needed.
    ...
    # Sort so Wuzzuf jobs appear first (if any)
    deduped.sort(key=lambda j: 0 if j["source"] == "Wuzzuf" else 1)

    print(f"[scraper] TOTAL jobs after dedup: {len(deduped)}")
    return deduped
